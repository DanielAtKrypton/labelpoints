function [h, ext] = labelpoints (container, xpos, ypos, labels, varargin)
...
if sum(strcmp('stacked', varargin))==1
    ...
    for s = 2:nlabs
        ...
        labhand = text(container, xpos(s-1)+u1 , ypos(s-1)+u2, labels(s-1), 'VerticalAlignment',va, 'HorizontalAlignment',ha, 'FontSize', p.Results.FontSize, 'Parent', axHand);
...
if multiColor
    ...
    for k = 1:length(labels)
        hand(k) = text(container, xpos(k)+u1 , ypos(k)+u2, labels{k}, 'VerticalAlignment',va, 'HorizontalAlignment',ha, 'FontSize', p.Results.FontSize, 'color', labelColors(k,:), 'FontWeight', p.Results.FontWeight, 'Parent', axHand);
    end
else
    %Label points all with 1 color (faster)
    hand = text(container, xpos+u1 , ypos+u2, labels, 'VerticalAlignment',va, 'HorizontalAlignment',ha, 'FontSize', p.Results.FontSize, 'color', labelColors, 'BackgroundColor', p.Results.BackgroundColor, 'FontWeight', p.Results.FontWeight, 'Parent', axHand);
end
extnt = get(hand, 'extent');

previousHoldState = ishold(axHand);
...
if p.Results.rotation~=0           %if rotation input is something other than 0  (use to be:  sum(strcmp(varargin, 'rotation')) == 1 )
    xl = xlim(axHand);      yl = ylim(axHand);                          %In case text rotation auto adjusts axes.
...
if previousHoldState ~= ishold(axHand)
    states = ["off", "on"];
    hold(axHand, states(previousHoldState+1));
end
end
